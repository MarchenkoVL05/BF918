...
function t806__init(recid) {
    ...
    var shareVK = testWrap.querySelectorAll('.t806__social-btn-vk');
    var shareFB = testWrap.querySelectorAll('.t806__social-btn-fb');
    var shareTwitter = testWrap.querySelectorAll('.t806__social-btn-twitter');
    ...
    if (shareVK) {
        for (var i = 0; i < shareVK.length; i++) {
            shareVK[i].addEventListener('click', function () {
    ...
    if (shareFB) {
        for (var i = 0; i < shareFB.length; i++) {
            shareFB[i].addEventListener('click', function () {
...
function t806__changeRadio(test, rightansw) {
    ...
    for (var i = 0; i < testInput.length; i++) {
        testInput[i].addEventListener('change', function (event) {
            ...
            var answerVote = event.target
                .closest('#rec' + test + ' .t806__answers')
                .querySelector('.t806__answer .t-vote__btn-res');
            ...
            if (event.target.getAttribute('type') === 'radio') {
                ...
                if (+testItem.querySelector('.t806__input:checked').value !== +currentRightAnswer) {
                    testItem
                        .querySelector('.t806__input:checked')
                        .closest('.t806__answer')
                        .classList.add('t806__answer_wrong');
                }
                // Описание details показываются на странице
                testItem.querySelector('.t806__input:checked').parentNode.nextElementSibling.style.display = 'block';
                testItem
                    .querySelector('.t806__input[value="' + currentRightAnswer + '"]')
                    .closest('.t806__answer')
                    .classList.add('t806__answer_correct');
                ...
                if (testItem.querySelector('.t806__input:checked').closest('.t806__answer_correct')) {
                    testItem
                        .querySelector('.t806__input:checked')
                        .closest('.t806__answer_correct')
                        .classList.add('t806__answer_withoutopacity');
                }
                if (testItem.querySelector('.t806__input[type="radio"]').closest('.t806__answer_correct')) {
                    testItem
                        .querySelector('.t806__input[type="radio"]')
                        .closest('.t806__answer_correct')
                        .classList.add('t806__answer_withoutopacity');
                }
            }
    ...
    for (var i = 0; i < testInput.length; i++) {
        testInput[i].addEventListener('change', function (event) {
            ...
            if (
                ...
                checkedAnswerCheck.indexOf(event.target.value) === -1
            ) {
...
function t806__checkClickBtn(test, rightansw) {
    ...
    for (var i = 0; i < testBtnCheck.length; i++) {
        testBtnCheck[i].addEventListener('click', function (event) {
            // Тоглинг кнопок check и next в карточке вопроса
            ...
            var answerVote = event.target
                .closest('.t806__btn-wrapper')
                .previousElementSibling.querySelector('.t806__answer .t-vote__btn-res');
            ...
            for (var i = 0; i < checkedAnswers.length; i++) {
                ...
                if (
                    +checkedCheckboxSort[i] === +checkboxAnswersArrSort[i] &&
                    checkedCheckboxSort.length === checkboxAnswersArrSort.length
                ) {
            ...
            if (testItem.querySelector('.t806__input').getAttribute('type') === 'checkbox') {
                for (var i = 0; i < checkboxAnswersArr.length; i++) {
                    testItem
                        .querySelector('.t806__input[value="' + +checkboxAnswersArr[i] + '"]')
                        .closest('.t806__answer')
                        .classList.add('t806__answer_correct');
                }
                for (var i = 0; i < checkedAnswers.length; i++) {
                    if (checkboxAnswersArr.indexOf(checkedAnswers[i]) === -1) {
                        testItem
                            .querySelector('.t806__input[value="' + +checkedAnswers[i] + '"]:checked')
                            .closest('.t806__answer')
                            .classList.add('t806__answer_wrong');
                        // Появление описания details в карточке вопроса 
                        testItem.querySelector(
                            '.t806__input[value="' + +checkedAnswers[i] + '"]'
                        ).parentElement.nextElementSibling.style.display = '';
                    }
                }
            }
            ...
            if (testItem.classList.contains('t806__lastquestion')) {
                event.target
                    .closest('.t806__question')
                    .querySelector('.t806__btn_result')
                    .classList.add('t806__btn_show');
            } else {
                event.target
                    .closest('.t806__question')
                    .querySelector('.t806__btn_next')
                    .classList.add('t806__btn_show');
            }
            testAnswers.classList.add('t806__answers_answered');
            // data-tilda-mode == null
...
function t806__restartClickBtn(test, rightansw) {
    ...
    for (var i = 0; i < testRestart.length; i++) {
        testRestart[i].addEventListener('click', function (e) {
            ...
            testBlock
                .querySelector('.t806__input')
                .closest('.t806__answer')
                .classList.remove('t806__answer_withoutopacity');
...
function t806__showResult(test) {
    ...
    var rightAnswersCount = document
        .querySelector('#rec' + test)
        .querySelector('.t806__counter')
...
function t806__openResultWrapper(testContainer, resultPercent, testBlock, fullResultLength) {
    if (testContainer.classList.contains('t806__result-new-calc')) {
        ...
        for (var i = 0; i < fullResultLength; i++) {
            if (
                resultPercent > (100 * (i + 1)) / fullResultLength &&
                resultPercent <= (100 * (i + 2)) / fullResultLength
            ) {
                testBlock.querySelector('.t806__result_' + (i + 2)).style.display = 'block';
            }
        }
...
function t806__showNumber(test, number) {
    ...
    for (var i = 0; i < testItemNumber.length; i++) {
        testItemNumber[i].innerHTML =
            '<span>' + number + '</span>' + '<span>/</span>' + '<span>' + testItemAll.length + '</span>';
    }
}
...
function t806_changeShareFBUrl(siteLocation, searchUrl) {
    ...
    for (var i = 0; i < params.length; i++) {
        if (
            params[i].indexOf('fb_action_ids') == -1 &&
            params[i].indexOf('fb_action_types') == -1 &&
            params[i].indexOf('result') == -1
        ) {
            url = url + params[i] + '&';
        }
    }
...
function t806_shareVK(recid, ptitle, purl) {
    ...
    var img =
        (dataForShare[2] || '').indexOf('://') != -1
            ? dataForShare[2]
            : window.location.protocol + '//' + window.location.host + urlPathname + slash + dataForShare[2];
